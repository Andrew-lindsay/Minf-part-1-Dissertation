...
\section{Evaluation}
The for-loop-rejuvenation defined in the previous sections will be evaluated, in a similar manor to the auto-rejuvenation with a comparison to an existing clang-tidy tool. The evaluation will compare what the transforms target, how they assess safety of transformation and there response to running on real world programmes.   

\subsection{Testing of rejuvenation and comparison to clang-tidy}
As previous mentioned in Section \ref{sec:cmp-auto-reju-mod-auto} and related work Section \ref{sec:rw-clang-tidy}, clang-tidy features a set of modernize checks. The most relevant for comparing to the for-loop-rejuvenation is the the modernize-convert-loop \cite{FOR_CONVERT}, previously mention in Section \ref{sec:rw-clang-tidy}. As the for loop types targeted by for-loop-rejuvenation, Section \ref{sec:forLoop-scope-trans}, were based on those defined by, the modernize-loop-convert check the comparison between the tools is made easier. Both transformations handle loops over statically allocated arrays, containers, using iterators and array-like containers, using at or []. The transforms differ though in how they assess the safety of a transformation and how open or restrictive they are in allowing loops into each category. 

The modernize-loop-convert implements a set of confidence level, risky, reasonable and safe, which dictate if the loop body is safe enough to allow for transformation of the loop. The risky confidence setting allows for loops to be transformed where methods of the container are called in the loop body while as well all all, reasonable confidence allows the conversion of those container which make calls to new \texttt{end()} or \texttt{size()}, similarly to the case shown in Figure \ref{fig:code-for-container}. Finally, the safe confidence level allows only the for loops that do not have criteria that would place them in the other two levels.

The testing of both tranformation was conducted on a 
...
% Problems in development - can be removed 

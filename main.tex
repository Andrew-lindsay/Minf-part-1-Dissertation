...
The previous example showed the impracticality of the transformation in some cases but this is not the only reason some for loops are ignored. In the file  \texttt{TraverseSchema.cpp} from \textit{xalancbmk} benchmark suffers from a similar situation of having many for loop and non being selected for transformaion. Figure \ref{fig:no-rejuv-indirect-var} shows one of \texttt{TraverseSChema.cpp}'s for loop header, the could actually be transform as the loop body (not displayed) has not conflicts and the loop is bounded by a call to the \texttt{size} method \texttt{fIC\_Elements}. However, currently in for-loop-rejuvenation this is rejected as the transform would not identify the loop header as array-like but statically array then error on not finding a integer value after analysing \texttt{icListSize}. This loop form could be handled if references to containers were excepted along with array-like loops also allowing variable in the comparison expression for size. The problem remains, how far should the transformation follow the expression to determine if a loop is met   
...
\section{Summary}
This chapter introduced the for-loop-rejuvenation and explored the transforms set of rules for detecting for loops and checking the 

% zero hit rate on most tests 
    
% write about data, what is shows

% try running the tests on multiple spec benchmarks and assessing the effectiveness discuss where it is lacking 

% oracle style try on selected files - not hard ? 

% give example of situation not caught code - bitboard
    
% clang get one better

% TEST 1 - TraverseSchema.cpp
    % unsigned int icListSize = fIC_Elements->size();
    %         for (unsigned int i=0; i < icListSize; i++) {

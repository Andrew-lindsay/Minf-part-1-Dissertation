...
\begin{figure}[H]
    ...
    \end{subfigure}
    \vspace{-0.5cm}
    \caption{Code along side the AST create by Rose Framework }
...
Starting with the C++11 standard, the language introduced type inference through the use of the \texttt{auto} keyword. This implementation of type inference it limited compared to other languages such as Haskell, as C++ \texttt{auto} can only be used for variable declarations \cite{HASK_TYPE_INF}. Unlike C++, Haskell's classical Hindley-Milner type inference allows for the complex type signatures of functions to be inferred from the body of the function. Figure \ref{fig:auto-type} is an example  showing  how some type signatures can be quite arduous, while not displaying any more useful information than stating the variable is an iterator.
...
\subsection{Comparison to Clang-tidy \& Testing }
...
for(TYPE it_name = CONT.begin(); it_name != CONT.end(); it_name++)
        \end{minted}
        \caption{Loop Type: Iterator over container}
    ...
    \end{minted}
    \caption{Loop Type: Array like container}
    \centering
    ...
    \end{minted}
    \caption{Loop Type: Statically allocated array}
    \centering
...
\subsection{Safety of transformation}
Assessing the safety of the transformation was the next significant challenge to handle, as certain uses of the index/iterator or container/array in the loop body could render the transformation to ranged-for invalid. The cases the transform aims to handle all have distinct ways the index/iterator or container/array are allowed to be used in loop body so are assess separately to assess safety.

\subsubsection{Safety: Iterator loop body}
...
The loop body requires changes to all old uses of the \texttt{init-expression} variable as they are now direct references to array elements and not index variables or pointers. The for loop iterator case requires the dereferencing of the iterator to be removed,and for the arrow expression notation to be replaced with the dot expression notation, Figures \ref{fig:before-tran-iter}, \ref{fig:after-tran-iter}. The for the loop types using indexing require a similar transformation with array/container being index need to be replaced as along with the index, Figures \ref{fig:before-tran-idx}, \ref{fig:after-tran-idx}.
...
The main entry point for the for-loop-rejuvenation is the traversal \texttt{ForLoopTraversal} which visits every for loop node in the source code file, checking if they match any of the three for loop types specified above.
...
The array-like container case and the statically allocated array case can share the same checks for a valid intialiser, \texttt{hasValidIndexInitialiser(..)}, and increment expression, \texttt{hasVaildIncrement}. The valid increment check can be the same check utilised by iterator for loop case as well as it handles overloaded and standard \texttt{++} operators. The shared check, \texttt{hasValidIndexInitialiser}, which ensures that the initial value of the indexing variable is 0 does not restrict to only allow standard integer zeros, it supports long integers, \texttt{0l, 0L}. The check also handles any implicit or explicit casting that can occur from the type utilised by the variable. The check rejects assigning the indexing variable any variation of a double value to simplify the process of extracting the value used to initialise it and compare it to zero.
...
The statically allocated array case required a unique approach to the comparitor check, \texttt{hasVaildBasicComparitor(..)}, as the index it compared to the size of the array before we have a reference to the container which is the focus of the for loop. To be able to check that the index is bounded by the size of the array, the array has to be known. So the hasVaildBasicComparitor function utilises the traversal ValidArrayIndexTraversal to scan the loop body of the for loop
...
% Over arcing structure
    ...
    %  actual transformation pass over the loop body 
        ...
        %  how replacement works, search for each use of index unsure that its parent node is the array access expression with the array as the container 
